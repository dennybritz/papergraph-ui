<script>
  ...
  const sortOptions = {
    Citations: papers => sortBy(papers, p => -p.num_citations),
    InDegree: papers => sortBy(papers, p => -$incomingCitations[p.id]),
    Year: papers => sortBy(papers, p => -p.year)
  };
...
<div>
  <div class="font-bold">{papersByCitations.length} papers in subgraph</div>
